...
// -----------------------------------------------------------------------------
//
// This DTO is retained for an explicitly authorized Session-provisioning
// operation.
//
// The CreateSessionCommand contract requires a
// SessionRefreshTokenHash:
//
//     refreshTokenHash: SessionRefreshTokenHash
//
// Therefore this DTO carries the already-derived refresh-token hash required
// by the explicit provisioning contract.
//
// A raw refresh token is deliberately NOT accepted by this DTO.
//
// This is important because the controller must remain aligned with the
// existing CreateSessionCommand contract:
//
...
// -----------------------------------------------------------------------------
//
// Transport primitives are converted before constructing
// CreateSessionCommand:
//
//     identityPublicId
//          ↓
...
//     ipAddress
//          ↓
//     SessionIpAddress | undefined
//
//     userAgent
//          ↓
//     SessionUserAgent | undefined
//
//     countryCode
//          ↓
...
//     correlationId
//          ↓
//     generated by controller with randomUUID()
//
//     causationId
//          ↓
...
// -----------------------------------------------------------------------------
//
// The DTO does NOT accept a raw refresh token.
//
// The distinction is:
//
//     raw refresh token
...
//     CreateSessionCommand
//
// The raw refresh token therefore does NOT enter this DTO.
//
// It does NOT enter:
//
// - CreateSessionCommand;
...
// - SessionRepository.
//
// The refresh-token hash is an opaque security representation.
//
// It MUST NOT be:
//
// - regenerated by the DTO;
// - interpreted by the DTO;
// - compared by the DTO;
// - modified by trimming or normalization.
//
// -----------------------------------------------------------------------------
//
// CORRELATION
// -----------------------------------------------------------------------------
//
// correlationId is intentionally NOT a DTO property.
//
// Direct HTTP operations generate their own correlation ID:
//
//     correlationId = randomUUID()
//
// The controller is responsible for supplying that value to
// CreateSessionCommand.
//
// -----------------------------------------------------------------------------
//
...
// -----------------------------------------------------------------------------
//
// causationId may be supplied by an upstream transport/application workflow.
//
// It is optional and is propagated when present.
//
// -----------------------------------------------------------------------------
//
// DOMAIN BOUNDARY
...
/**
 * Trims ordinary transport strings before validation/mapping.
 *
 * Security-sensitive opaque values such as refreshTokenHash are deliberately
 * NOT transformed.
 */
...
const MIN_REFRESH_TOKEN_HASH_LENGTH = 1;
const MAX_REFRESH_TOKEN_HASH_LENGTH = 4096;
 ...
 *
 * The application boundary converts these primitive transport values into
 * domain value objects before constructing CreateSessionCommand.
 *
 * IMPORTANT:
 *
 * This DTO accepts refreshTokenHash because CreateSessionCommand explicitly
 * requires SessionRefreshTokenHash.
 *
 * It does NOT accept a raw refresh token.
 */
export class CreateSessionRequestDto {
   ...
   *
   * The corresponding CreateSessionCommand property is explicitly typed as:
   *
   *     SessionDevicePublicId | undefined
   */
  @ApiPropertyOptional({
  ...
  /**
   * Already-derived refresh-token hash.
   *
   * This is NOT a raw refresh token.
   *
   * The controller converts this primitive into:
   *
   *     SessionRefreshTokenHash
   *
   * before constructing CreateSessionCommand.
   *
   * IMPORTANT:
   *
   * This value is intentionally not passed through trimString.
   *
   * Cryptographic/security representations are opaque values and must not be
   * silently modified at the transport boundary.
   */
  @ApiProperty({
    example: '$argon2id$v=19$m=65536,t=3,p=4$example$example',
    description:
      'Already-derived refresh-token hash required by CreateSessionCommand. This is not a raw refresh token and must not be modified or normalized.',
    minLength: MIN_REFRESH_TOKEN_HASH_LENGTH,
    maxLength: MAX_REFRESH_TOKEN_HASH_LENGTH,
    writeOnly: true,
  })
  @IsString({
    message: 'refreshTokenHash must be a string.',
  })
  @MinLength(MIN_REFRESH_TOKEN_HASH_LENGTH, {
    message: 'refreshTokenHash must not be empty.',
  })
  @MaxLength(MAX_REFRESH_TOKEN_HASH_LENGTH, {
    message: `refreshTokenHash must not exceed ${MAX_REFRESH_TOKEN_HASH_LENGTH} characters.`,
  })
   ...
   *
   * This is an opaque identifier and is not itself a token secret.
   */
  @ApiProperty({
   ...
   *
   * This identifies the command, domain event, or operation that caused this
   * Session-creation operation.
   *
   * correlationId is deliberately absent from this DTO because direct HTTP
   * operations generate correlation IDs at the controller boundary.
   */
  @ApiPropertyOptional({
...
export {
  ...
  MIN_REFRESH_TOKEN_HASH_LENGTH as SESSION_REFRESH_TOKEN_HASH_MIN_LENGTH,
  MAX_REFRESH_TOKEN_HASH_LENGTH as SESSION_REFRESH_TOKEN_HASH_MAX_LENGTH,
